...
following security guarantee: if the \https{fbi.gov} is honest, then we
...
it opens an iframe to \https{fbi.gov}, exercising its privilege to keep
its initial label as \https{fbi.gov} so it can (2) talk to the \https{fbi.gov}
...
its label to ``\https{gdoc.com} and \https{fbi.gov}'' to read the decryption.
...
with the \https{fbi.gov} privilege, all documents must pass through it for
