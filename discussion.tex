...
deciding to trust a mashup integrator such as \https{mint.com} they
are only trusting the app to not leak their data through covert
channels.


\Red{HERE}
\todo{ds}{mashups that care about user privacy yet themselves rely on
third party code can use clearance to privilege seprate their code and
ensure that this third party code cannot access data it does not need}


%
As mentioned in Section~\ref{sec:system}, \sys{} fully implements the
notion of clearance, previously developed in
~\cite{efstathopoulos:asbestos, Zeldovich:2006, stefan:2011:flexible,
Breeze13}, as a way to restrict what data piece of code can read and
thus exfiltrate.
%
