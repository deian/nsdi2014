...
%
Although this approach capture a wide-set of confinement policies, it is not
suitable for scenarios in which a circular flows of information is
desired~\cite{Badger:1995}, or when more powerful logics (e.g., first order
logic as used by Nexus~\cite{sirer2011logical}) are necessary.
%
...
Although DC labels are as expressive as other label
models~\cite{GenLabels}, including Myer's and
Liskov's~\emph{Decentralized Label Model}~\cite{dlm}, we
primarily chose DC labels because of their close correspondence to web
origins.
%
...
%
(Recall the expressiveness of today's policies is mostly limited to
origin-whitelists.)
%
