...
%
All our measurements were conducted on a 4-core i7-2620M machine with
16GB of RAM\@ running GNU/Linux 3.13. We used the Node.js web server
over a loopback interface for all applications.
%
Before delving into the benchmarks, however, we summarize
the implications of the results:
\begin{CompactItemize}
  \item COWL's impact on legacy sites is negligible.
  \item Confining code with LWorkers is extremely cheap, especially
    when compared to iframes/Workers.
    %
    Indeed, the performance of our end-to-end confined password
    checker is only 5ms slower than an inlined \js|script| version.
    %
    While already unperceivable, this cost becomes even more
    negligible when accounting for network latency, DOM
    rendering times, \emph{etc.} of real-world applications.
    %
    (Our apps run on loopback, do not use the DOM in an interesting
    way, nor perform any heavy computations.)
  \item The impact of COWL's confinement informant on mashups is minimal.
    %
    Our highest overhead is 16\%, for the encrypted document editor.
    %
    Again, the absolute slowdown of 16ms is unperceivable for users;
    when accounting our setup (serving content over loopback with
    no interesting DOM manipulation), this figure is negligible.
\end{CompactItemize}
