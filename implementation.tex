...
%
Unfortunately, the lack of wrappers has the additional implication
that shared references cannot be revoked.
%
Hence, before enabling confinement mode, a page can create a
same-origin iframe with whom it shares references; the iframe can
thereafter leak any data from the parent even if the latter's label is
raised.
%
To mitigate this, our current Chromium API allows senders to disallow
unlabeling Blobs if the target created children before entering
confinement mode.
