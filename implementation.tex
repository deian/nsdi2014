...
interface LWorker :
  ...
  \end{subfigure}
  \caption{\todo{ar}{Why the or is bolded?} \sys{} programming interface in simplified WebIDL.}
  \label{fig:api}
\end{figure}
...
\todo{ezy}{maybe flip this section after}
...
%
As shown in Figure~\ref{fig:api}, the \js|COWL| object exposes these
...
\todo{ezy}{A diagram would be nice here! I don't think it's really clear from the text above how wrappers work.}
...
Figure~\ref{fig:api}---we ``recompute'' \todo{ezy}{Explain why recomputing is
  desirable!} 
\todo{ar}{Agreed, perhaps ``SWAPI reaccomodates all the cross-compartment 
wrappers to enforce the restrictions imposed by the current label, and not the 
SOP''}
all the cross-compartment
...
%
To this end, we implemented a new wrapper policy \todo{ezy}{What's a wrapper policy} and modified the
...
confinement-mode is enabled). \todo{ezy}{As well as the penalty of the COWL check, no?}
