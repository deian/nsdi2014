...
Similar to the \js|postMessage| API, CORS~\cite{cors13} gives
...
origin in a new compartment. The label of the new compartment is
initially \js|public|. First, in step (1), the checker script is free
...
allowed to store encrypted documents provided by \js|nsa.gov|.
...
once it has read the responses from those origins.\footnote{\Red{MOVE: The
  ...
  addresses the shortcomings of DAC. %meets this need.
...
This highlights the need for a confinement system that supports
{\em delegation}: a page should be allowed to both, deem another
script trustworthy, and, at any point, deem itself untrustworthy.
...
\item Supports {\em declassification,} i.e., it allows principal $A$
  to declassify data sensitive to $A$ (but, of course, not $B$);
\item Supports {\em delegation,} i.e., it allows principal $A$
  to grant another principal the ability to ``speak, '' i.e., 
