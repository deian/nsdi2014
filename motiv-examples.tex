...
browser should \todo{ar}{Is this a OS term? why not just protect?} robustly protect the user's sensitive data from
unauthorized disclosure, yet afford page developers the greatest
possible flexibility to construct \Red{rich, }featureful applications that
reuse functionality implemented in scripts provided by (potentially
distrusted) third parties. \todo{ar}{too long relative closure} To make concrete the diversity of potential
...
with \todo{ar}{why not just privacy?, what does it mean ``strong privacy''} strong privacy for the user in today's web browsers. These
...
prior to our work. \todo{ezyang}{I think this could be made shorter}
...
``plugin'' library she trusts to perform cryptography \todo{ar}{what is ``robust
  cryptography''?}robustly. In this design,
...
script so that it cannot exfiltrate the document to any origin {\em
  other} than \js|google.com|. \todo{ar}{It is not clear what is the 
concern of nsa. The crypto library does not care about the exfiltration of 
the  plaintext since it got it from google already. Is nsa giving some data to 
google more than the cyphertext?}
...
\todo{ar}{Is this server-side mashup? If so, we should say it explicitly}
Alternatively, a
...
once it has read the responses from those origins.\footnote{The desired
semantics here are very subtle. Should a third-party script be continue
to be able to communicate with the origin which it \emph{first} read
sensitive data? In some cases, this could lead to the possibility
of cross-site request forgery.  We explain the correct semantics in
detail in Section~\ref{sec:labeled-xhr}} Here, too,
...
these untrusted libraries.
A primary difficulty is that jQuery requires wide reign over the
contents of the entire web page, which means it is not reasonable to try
to confine jQuery to an isolated context in the same way we isolated the
password checker.  The key insight is to flip the situation: the trusted
code should be placed in another context, so that it can manipulate the
(untrusted) page running jQuery, but not vice versa.  Once more,
MAC-based confinement intuitively seems to fit well.
