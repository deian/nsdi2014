...
DOM. We use the term \emph{compartment} to refer to both browsing
...
To limit the damage of cross-site scripting (XSS) attacks, wherein
malicious code is injected into a page, developers can tighten-down
the SOP with CSP~\cite{csp}.
%
With CSP, developers set various directives, white-listing origins
from where different content (images, scripts, etc.) can be loaded.
%
This has the consequence of restricting to whom data can be leaked
through content loading.
%
Other CSP directives explicitly limit data exfiltration.
%
...
to navigation.
%
Worse yet, CSP policies are static, i.e., policies cannot change
through the lifetime of a page (e.g., if the page wishes to drop
communication privileges with certain hosts), and transparent, i.e., a
context cannot inspect the policy of another context (e.g., to decide
if it wishes to share sensitive data with it).

\paragraph{Cross-Origin Resource Sharing}
To achieve more flexibility, CORS~\cite{cors13} extends the XHR object
to provide developers with a means for loosening the SOP.
%
...
%
Unfortunately, CORS only gives a host control over the origins that
are allowed to read a resource---once access is granted, they have no
control over what the cross-origin code can do with the data.
