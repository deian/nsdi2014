...
%
With CSP, developers set various directives, white-listing
origins from where different content (images, scripts, etc.) can be
loaded.
%
This has the additional consequence of restricting to whom data can be
leaked through content loading.
%
Moreover, certain CSP directives are used to explicitly limit data
exfiltration.
%
For example, the \texttt{connect-src} can be used to white-list
origins that the page is allowed to perform XHR requests to.
...
to navigation.
%
Worse yet, CSP policies are static, i.e., they cannot change through
the lifetime of a page, and ``write-only,'' i.e., one context cannot
inspect the CSP of another to, for instance, decide if it wishes to
