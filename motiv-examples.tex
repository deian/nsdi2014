...
the SOP with content security policy (CSP)~\cite{csp}.
%
...
%
Other CSP directives limit data exfiltration through other
communication channels.
%
For example, the \texttt{connect-src} directive can be used to
white-list the origins that the page is allowed to perform XHR
requests to.
...
through the lifetime of a page (e.g., if the page wishes to no longer
communicate with certain hosts), and ``write-only,'' i.e., one context
cannot inspect the policy of another context to, for instance, decide
