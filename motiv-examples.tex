...
ordinarily be permitted by the default SOP\@.  \Red{I can't parse
  previous sentence.  Is it the page (nearest antecedent) that
  protects the user's privacy or the CSP\@?}  Developers may set
...
under \sys{}.
\Red{The flash looks like it is part of step 3, should it be step 2?}
}
\end{figure}
...
the cryptography library (say, \js|nsa.gov|).
\Red{That joke could confuse people.  Maybe better to make the intent
  clear\--e.g., \texttt{eff.org}.}
Note that these two
