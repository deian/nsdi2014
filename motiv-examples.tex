...
architecture.\todo{ds}{update with fig that shows DAC}}
\vspace{-10pt}
\end{figure}
Figure~\ref{fig:primer-browser-arch} highlights the main components of
the current web security architecture.
%
Core to this is architecture is the notion of \emph{browsing
contexts}, i.e., pages and frames.
%
A browsing context encapsulates presentable content and a JavaScript
execution environment (heap and code) that interacts with content
through the \emph{Document Object Model (DOM)}~\cite{html5}.
%
...
DOM objects).
%
Some contexts such as Web Workers run JavaScript but do not have a
DOM. We use the terms \emph{contexts} and \emph{compartment}
interchangeably to refer to both browsing contexts and workers when
the more precise meaning is not relevant.
...
it can only leak data through image loading to this (i.e.,
\https{b.com}) origin.)
%
...
inter-compartment messaging (e.g., via \js|postMessage|) or leaks due
to navigation.
%
...
%
More fundamentally, CSP policies are 1) \emph{static}, i.e., policies
cannot change through the lifetime of a page (e.g., a page cannot drop
...
\paragraph{postMessage \& Cross-Origin Resource Sharing}

Within the confines of the browser, the HTML5 \js|postMessage|
API~\cite{webmessaging} gives developers the flexibility of
implementing applications that can exchange messages between
cross-origin compartments, as shown in Figure~\ref{fig:primer-browser-arch}.
%
When sending a message the intended origin of the message must be
specified to ensure that an arbitrary context cannot read the message.
%
...
Similar to th \js|postMessage| API, CORS~\cite{cors13} gives
developers the flexibility of implementing applications that can read
data from a cross-origin server.
%
...
%
Recall that the SOP would not otherwise allow code to inspect a
cross-origin response.
 
Unfortunately both, the \js|postMessage| intended origin, and CORS 
solely implement discretionary access control.
%
In the former case, a sending compartment can control the origin that
that is allowed to received the message, but has no control over what
the receiving context can do with the received data.
%
In the latter case, a host host can control the origins that are
allowed to read the resource, but once access is granted, they have no
control over what the cross-origin compartment can do with the data.
%
...
remote origin.
%
Yet, a CSP policy that permits such communication event with the
top-level page's same origin is \emph{too} permissive: a malicious
script could carry out a \emph{self-exfiltration
attack}~\cite{Yip:2009:PBS,selfex}, and write the password to a public
page at that trusted origin.
...
numbered sequentially in time. Following~\cite{yang:2013:towards}, compartments may be
\emph{labeled} (Section~\ref{sec:labels}) with the
...
once it has read the responses from those origins.\footnote{The
  ...
  forgery~\cite{CSRF}. We explain the correct semantics in detail in
