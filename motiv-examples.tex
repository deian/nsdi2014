...
written by different authors.\footnote{Throughout we use ``web page''
  ...
  ``script'' interchangeably.} \Red{EZY suggests cutting: Ideally, the browser should protect the
...
suffer from two limitations. The first limitation is that they are {\em static:} they
...
reading potentially sensitive data). The second limitation is that they are {\em inaccessible:}
...
libraries like jQuery \Red{EZY suggest cut: (whose prevalence we cite in the
...
communicate with remote origins on that compartment, after which it gives up
its own privileges.
