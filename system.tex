...
%
In the jQuery example, the application delegate a child context
its privileges before loading jQuery; this allows
the context to act on behalf of the origin while the main browing
context is confined.
%
We remark that with these capabilities we can additionally provide a
programming model wherein privileges must be explicitly exercised as a
library that wraps functions, such as \js|postMessage|, which sets and
