...
%
Below, we extend our notion of first-class confinement to iframes
(sometimes referred to as labeled iframes) by describing the design of
a mostly untrusted password manager---an application that allows
...
%
The third party, \https{mint.biz}, may wish to present the user with a
...
\https{mint.biz} from accessing the user's \https{amazon.com} and
\https{bank.ch} data, e.g., using \xhr{}.\footnote{
 ...
 explicitly set CORS headers~\tocite{CORS} to give \https{mint.biz}
 access to such data.
}
%
However, with \sys{}, we can allow \https{mint.biz} to access such
...
We note that our modified XHR object allows the response of a
...
\https{bank.ch}, since the application may need to, for example, fetch
