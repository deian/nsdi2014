...
must create a iframe, which would have the page privilege and thus be
able to communicate arbitrarily.
%
In this scenario, child frame serves the role of a ``trusted''
component (akin to OS kernels).
%
That is, the iframe it will carry out any request (submitted via
\js|postMessage|) on behalf of the parent, after asserting that the
request is safe (e.g., update local storage to reflect the last page
view, etc.).
