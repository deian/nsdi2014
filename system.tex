...
arbitrarily~\tocite{saltzer}.
%
The confinement mechanism allows developers to enforce both, policies
...
and CORS.
%
In this section, we first describe how policies are encoded in \sys{}
...
%
We chose DC labels, much like Hails~\tocite{hails} and
Breeze~\tocite{breeze}, because they are as expressive as labels used
by other practical confinement systems~\cite{GenLabels}, but have a
...
statements with purchases.
%
Importantly, the label specifies that the data should not be
propagated to a less sensitive entity, e.g., an \https{amazon.com}
server, which---like all end-points---is implicitly labeled using its
origins (in this case, \js|Label("https://amazon.com")|); \sys{}
prevents such dissemination of data since bank data may otherwise be
leaked.
%
Of course, if the bank wishes to share data with \https{amazon.com} it
can label it \js|Label("https://bank.ch").or("https://amazon.com")|;
this allows both parties to read and disseminate the data.
...
\js|subsumes| shown in Table~\toref{table:components}.
%
Specifically, data labeled \js|ldata| can be sent to an entity labeled
\js|lentity| only if \js|lentity.subsumes(ldata)|.
%
Intuitively this states that an entity is allowed to receive
data that is at most as sensitive, i.e., the entity label
\js|subsumes| the label of the data;
%
we refer the interested reader to~\cite{stefan:2011:dclabels} for more
details.

In \sys, we associate a label with every
\emph{compartment}~\tocite{franz}, rather than heterogeneously with
every object.
%
A compartment corresponds to the global object of a web page, iframe,
Worker~\tocite{}, etc.
%
We loosely use the term \emph{browsing context} to mean the
compartment (environment) and (content or JavaScript) code executing
...
\https{amazon.com}.
%
In turn, \sys{} uses this label to confine code by restricting writes
from the browsing context.
%
Specifically, code is restricted to
communicating with other browsing contexts (e.g., using
\js|postMessage|) whose labels subsume the current label.
%
Similarly, performing network requests and accessing persistent
storage is dictated according to the context label and implicit labels
of these end-points.

\sys{} makes SOP's notion of authority of page content explicit, using
\emph{privileges}.
%
Specifically, privileges are unforgeable objects with which code can
assert the authority of their corresponding origins to declassify
that origin's data.
%
In \sys, scripts are granted the privilege corresponding to the
page origin.
%
In turn, the privilege can be used when performing the \js|subsumes|
label check to bypass restrictions imposed by any component subsumed
by the privilege.
%
For example, a script running in an \https{amazon.com} page has the
privilege corresponding to the \https{amazon.com} origin,
which can be used to downgrade the label
from \js|Label("https://bank.ch").and("https://amazon.com")| to
\js|Label("https://bank.ch")|; by changing the label of a browsing
context as such, we allow code to 
communicate with additional browsing contexts and event send XHR
requests to \https{bank.ch}.
 
Importantly, code cannot synthesize arbitrary privileges.
%
For example a script executing in a \https{amazon.com} browsing
context cannot create a privilege for \https{bank.ch}---this would
trivially violate any confinement guarantees.
%
In our system, code can only create the empty privilege (which is
simply the JavaScript \js|null|) and privileges corresponding to
fresh, unique origins (create with \js|new FreshPrivilege()|).
%
As in HiStar~\tocite{histar}, the ability to generate fresh
privileges, in addition to having the ability to grant them, ensures
that all browsing contexts are egalitarian, i.e., a child iframe is no
less trustworthy than its containing parent.
...
%
Hence, as in HiStar~\tocite{histar}, Hails~\tocite{hails}, and
Breeze~\tocite{breeze} we associate a \emph{clearance}---which is
simply a label---with every compartment, imposing a limit on the kind
of data a compartment can have, and thus a piece of code can access.
...
%, executing in an
%LWorker, have a sufficiently high context label such that it 
%% To far from previous section, I believe the idea is clear.
%otherwise \sys{} would not dispatch the
%password message.
%
...
site-specific passwords.%---as a website.
...
(e.g., from 
\https{fb.com}) or trust arbitrary websites (e.g.,
...
\item The management layer never learns the site-specific (\https{fb.com})
  ...
  extension, describe in Section~\toref{XXX}).
...
\item The storage layer never learns the site-specific credentials or master
  ...
  management layer. \Red{A realistic assumption is decentralized environment
    like the web.}
...
communicate outside the \https{fb.com} and \https{dropcu.be} layers.\footnote{
  ...
  \https{fb.com}. Importantly, the privilege must be granted to the storage
  layer.  }
\Red{Ale: can it be learned by the storage layer?}
...
%
\Red{don't get it, the manager has label fb or dropcu.be so we can just send a 
message with exactly the label dropcu.be}
...
%
Since the storage layer frame has the \https{dropcu.be} privilege, it does not
...
similar to the one showed below. \Red{for what? to react when getting the 
