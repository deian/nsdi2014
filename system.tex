...
%
In the jQuery example, the application delegates a child context
its privileges before loading jQuery; this allows
the context to act on its behalf, while leaving the main context 
confined.
%
We remark that with these capabilities we can also provide a
programming model wherein privileges must be explicitly exercised as a
library that wraps functions, such as \js|postMessage|, to set and
