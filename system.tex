...
Breeze~\tocite{breeze}, because they have a simple and clear
...
%
As in HiStar~\tocite{histar}, the ability to generate fresh privileges
...
attack~\tocite{self-exfiltration} and leak the password to a public
...
public label).
%
Our use of labeled workers in this fashion is similar to the
executing code in LIO's \js|toLabeled| block~\tocite{lio} and
Breeze's \js|bracket|~\tocite{breeze}.
...
\https{bank.ch} data, e.g., using \xhr{}.\footnote{
 ...
 explicitly set CORS headers~\tocite{CORS} to grant \https{mint.biz}
 access to such data.
}
%
