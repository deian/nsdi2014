...
\dcLabel{\https{amazon.com}}{\https{mint.com}} can raise its label to
\dcLabel{\https{amazon.com} $\land$
...
%
First, since COWL applications are simply regular web applications
that additionally use the COWL API, an application can create standard
(but labeled) contexts in the form of pages, iframes, workers, etc.
%
Importantly, however, all pre-existing communication channels---even
subtle channels exposed through the evolving DOM API---must be
mediated according to the context label to ensure confinement.
%
In Section~\ref{sec:implementation} we expand on how COWL does this.
