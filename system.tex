...
\Red{Every compartment} has a \emph{current label} which protects all data associated with the compartment or
web site, and specifies what origins can read the data.
%
...
\begin{itemize}
    \item A privilege can be used to \emph{declassify} private
        ...
        ``public'' \todo{ar}{The next line could be removed} 
        ; we could even downgrade it to ``\https{amazon.com}
...
\todo{ar}{Does it mean that when creating labeled blob privileges are not applied?}
To force a label
...
%
Importantly, any code can allocate a \todo{ar}{a pair of privileges? not clear
  the ``type'' of the privilege} origin-privilege pair: this
