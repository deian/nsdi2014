...
context label is at least as sensitive as \js|l|---thereby preserving
the privacy of the \https{instagra.me} password.

Dually, \sys{} ensures that the privacy of data sent by an
LWorker (via \js|postMessage|) is preserved by its parent.
%
...
%
In the case above, the context label remains unchanged when
registering the handler since the parent has the privilege
corresponding to the page origin used to label the worker.
%
If the page did not own the \https{instagra.me} privilege, the context
label would be raised to the label of the worker \js|l|;
%
...
\end{jscode}
%
\todo{ds}{note that in this case you can only fetch dictionary from
CORS-allowed pages}
