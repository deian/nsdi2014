...
%
As with loading content, writing to storage (including cookies), or
performing explicit network requests with \xhr{}, \sys{} asserts that,
given the current context privileges, the label of the
resource---which, as in other end-point cases, is implicitly
\js|Label(|\emph{resource origin}\js|)|---subsumes the context label.
%
In this case, \js|Label("https://checker.ru")| subsumes the public
context label, hence performing the request will not leak any data and
thus \sys{} fetches the script.

Before executing executes the fetched script in a new browsing
context, as shown in Figure~\toref{fig:strength-1}, \sys{} ensure that
the label of the worker is at least as restricting as the current
label.
%
Just as the context label protects all data in scope, the label of a
worker protects the data within the worker, including messages sent to
it. 
%
Hence, our check ensures that the page cannot ``declassify'' data by
