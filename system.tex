...
\begin{webidl}
LWorker {
  LWorker LWorker(String, Label[, Privilege, Window])
...
bank privilege and thus be able to communicate arbitrarily.
%
...
Unfortunately, such compartmentalization is not trivial for 
realistic scenarios.
%
This is primarily because the trusted iframe cannot inspect the DOM
(state) of the parent frame and make decisions without falling prone
...
%
Without knowing a-priori the URL of the source code, or even the order
in which requests should be made, the trusted frame, must decide
whether fetching the source will leak any information---a complex
decision we wish to avoid imposing on developers!

Addressing this, we allow developers to create labeled workers that
have access to the page DOM and page-origin privilege.
%
(We naturally call labeled workers with access to the DOM:
...
argument.
%
This, for example, allows \https{bank.ch} to create a labeled DOM
worker that has access \https{bank.ch} privilege before completely
...
%
We note that, different from the iframe approach, \https{bank.ch} need
not implement a complex protocol between the code executing in the
...
%
\sys{} ensures that all DOM access is mediated to ensure that no
information can leak between the compartments when the worker performs
an operation.
%
Specifically for all \js|get|, \js|set| and \js|call| operations we
ensure that the both the page label subsumes the label of the worker,
taking into considering the worker's privileges, and vice
versa---i.e., the label of the worker and page must be the same, when
considering the worker's privileges.
%
In the above example, the bank \js|tcb| worker is able to inspect and
modify the DOM because the worker effective label, i.e., label when
considering privileges, equals that of the parent page.
%
Note that we also set the page clearance to ensure that a malicious
jQuery cannot raise the page label and thus ``revoke'' the worker's
access.
%
We remark that we are conservative in treating every DOM access as
having a read and write effect, since both \js|get| and \js|call| may
call a (e.g., getter) function that modifies state.\footnote{
