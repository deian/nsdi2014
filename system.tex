...
ensures that all browsing contexts are egalitarian, i.e., a child
iframe is no less trustworthy than its containing parent.
...
\begin{webidl}
Browbound {
  attribute Label privacyClearance
}
\end{webidl}
&
...
\\\hline %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\end{tabular}
\caption{\label{systemAPI} \sys{} design components and
  API\todo{ale}{make sure to say in the caption that the API is
described in a WebIDL-like language}}
\end{table*}
...
Browbound.privileges = p.combine(Browbound.privileges);
...
Rather than attempt to solely confine jQuery within such a page, a
first conservative approach is to confine the whole page. 
%
To this end, \sys{} can be used to raise the label of the browsing
context and drop the page privileges before importing the library:
\begin{jscode}
// Raise label:
...
\end{jscode}
%
This ensures that the page can only leak data to \https{bank.ch}.
%
Consequently, the jQuery library code must be also be hosted on the
same origin.
%

To allow the page to communicate with other domains, the application
must create an iframe, which would have the bank privilege and thus be
able to communicate arbitrarily.
%
The role of this child frame is to solely implement ``trusted''
components of the application.
%
Similar to a OS kernel, the iframe it carries out any requests
(submitted via \js|postMessage|) on behalf of the caller, after
asserting that the request is safe (e.g., updating local storage to
reflect the last page view, etc.).

Unfortunately, such structuring of an application, for any realistic
scenario, is not trivial.
%
For example, to ensure that the library cannot carry out
self-exfiltration attacks we must set the context label to the label
of a fresh principal.
%
This, in turn, requires granting the privilege to the trusted
child iframe (as to not restrict the child from performing arbitrary
requests), and requesting it to fetch the library source code.
%
Without knowing a-priori the URL of the source code, the trusted
frame, must decide whether fetching the source will leak any
information---a complex decision we precisely wish to avoid imposing
on developers!
