...
{\em Labeled browsing contexts} enforce MAC-based confinement of
  JavaScript at the granularity of a browsing context (e.g., a frame
  ...
  sensitive information to unauthorized parties. For example, {\em
    declassification} of sensitive data strips the data's label, after
  which any scripts in other compartments sent that unlabeled data
  will not be confined therein. In COWL we express privilege in terms
  of origins. The crown icon in the left compartment in
  Figure~\ref{fig:checker} denotes that this compartment holds the
  privilege to execute these certain operations on data labeled with
  the origin {\tt fb.com}.
...
\subsection{Labeled Compartments}
Labels as used to restrict communication. Relate to hierarchical
confinement; compartments may be nested to arbitrary depth.
- Labeled blobs. Tie back to comparison with postMessage.
\paragraph{Confining Pages and Workers}
\paragraph{LWorkers}
