...
violate the secrecy of that origin's data, e.g., because the browser
...
%
Our third-party library example uses such a \emph{DOM worker} \todo{}{This
  concept is not introduced before} to
...
not the payload\todo{}{We should clarify that inspecting the label 
does not reveal information, i.e., sensitive information cannot be 
encoded in the label (since we checked that the label on the label already 
flows to the label of the destination context)}. Only after raising its label as needed may the
...
\https{nsa.gov} (i.e., in a compartment holding the \https{nsa.gov}
privilege) with a current label \dcLabelS{\https{nsa.gov} $\land$
  \https{gdoc.com}}{} is permitted to send messages to a context
labeled \dcLabelS{\https{gdoc.com}}{}.
%
Without the \https{nsa.gov} privilege, this flow would not be allowed,
as it may leak the NSA's information to Google.
%
%% NSA is declassifying the message by using privileges. i.e., it
...
% declassifies data labeled with that origin upon receipt---i.e., it
...
(e.g., when sending messages) may be
safer~\cite{Zeldovich:2006}~\tocite{lio}, we find it a poor fit with
...
%
COWL instead lets code control the use of privileges by allowing code
...
%% Doing so is useful, for example, when confining a context that
%% contains (intrusive) untrusted libraries such as jQuery; here, we drop
%% the context privilege to ensure that jQuery cannot act on behalf of
%% the origin and arbitrarily leak data.
%
...
%
COWL also allows a context to create privileges for \emph{fresh}
origins, i.e., unique origins that do not correspond to a real URI
