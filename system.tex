...
server, which---like all end-points---is implicitly labeled with it
origins (in this case, \js|Label("https://amazon.com")|); \sys{}
prevents the dissemination of such data to \https{amazon.com} (either,
in-browser or with XHR), since it may leak bank information.
%
...
such disjunctive label states that either origin may disseminate the
data.
...
every \emph{compartment}~\tocite{franz}.
%
A compartment roughly corresponds to the JavaScript heap (for
simplicity, global object) of a web page, iframe, Worker~\tocite{},
etc.; this allows us to reason about JavaScript security semantics at
a coarse-grain level---we need only impose security checks for
...
compartments do not have a browsing context~\tocite{workers}.
...
\sys{} makes SOP's notion of the authority of code over page content
explicit using \emph{privileges}.
%
Privileges are unforgeable objects with which code can
assert the authority of their corresponding origins to declassify
that origin's data.
%
In \sys, the code in a browsing context is given the privilege
corresponding to the page origin.
%
...
XHR requests to \https{bank.com};
%
in doing so, the code is effectively declassifying any
\https{amazon.com}-sensitive data used in the request.
...
data a piece of code can access.
...
simple third-party strength checker, \https{checker.ru/checker.js}.
%
...
do not have server-side access beyond content hosting.
%
...
%
In general, a script can explicitly enable confinement-mode and set
the compartment label to a label more restricting than the current
label; when enabling confinement-mode implicitly, \sys{} takes a
permissive approach and sets the current label to the public label.
 
Subsequently, \sys{} ensures that the current page is allowed to make
...
%
In this case, the resource label \js|Label("https://checker.ru")|
subsumes the context label \js|Label()| and thus fetching the script
is permitted.

Before executing the fetched script in the worker compartment, \sys{}
...
label.
%
Then, as shown in Figure~\toref{fig:strength-1}, \sys{} starts
...
information.
%
Only once the code has decided to inspect the password should it no
...
%
(If an explicit label is not supplied, \sys{} uses the compartment
label of the sender as the message label.)
%
We use this label to ensure that the message is dispatched to the
checker only if its compartment label is at least as sensitive as
