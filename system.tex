...
information---a complex decision we wish to avoid imposing on
developers!

Instead, we allow developers to create labeled workers that have
access to the page DOM and page-origin privilege.
%
As noted in Section~\ref{sec:system:worker}, the \js|LWorker|
constructor can be supplied with a \js|Privilege| and \js|Window|
argument.
%
This, for example, allows \https{bank.ch} to create a labeled worker
that has access to the DOM and the \https{bank.ch} privilege, before
completely confining the page:
\begin{jscode}
var tcb;
(function() {
  var p = new FreshPrivilege(),
...
\end{jscode}
%
Since untrusted code will be imported into the context, we use a
