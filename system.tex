...
parties. For example, if a script {\em declassifies} messages it
receives from another context, it will retain the ability to do as it
pleases with the data received in those messages---i.e., the sending
context will not have confined the receiving context. We refer to such
access-controlled operations in COWL as {\em privileged}, and the
right to invoke them as a {\em privilege.} One must trust the script
invoking a privileged operation not to abuse it. In COWL we express
privilege in terms of origins. The crown icon in the left compartment
in Figure~\ref{fig:checker} denotes that this compartment holds the
privilege to execute privileged operations on data labeled with the
origin {\tt fb.com}. In Figure~\ref{fig:checker}, the {\tt fb.com}
compartment uses its privilege to remain unconfined by declassifying
the checker response labeled {\tt fb.com}.
