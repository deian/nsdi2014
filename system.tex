...
handled, and (3) \sys{} provides a variant of this API called
\emph{labeled XHR} which relaxes the same-origin policy.
...
Finally, we consider the design of \emph{labeled XHR}.
%
...
forgery.
%
Thus, labeled XHR returns its result as a labeled blob, protected
with a server-specified label that defaults to a fresh origin
(Section~\ref{sec:privileges}).
%
