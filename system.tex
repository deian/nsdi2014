...
%
These labels are used to restrict how information can flow between
contexts, usually in addition to the restrictions imposed by existing
policies such as the SOP.

Following~\cite{yang:2013:towards, stefan:2011:dclabels}, a label is a
pair of boolean formulas over origins, specifying secrecy and
integrity restrictions.
%
The secrecy formula of a context label indicates the
sensitivity of the data in the compartment, while the integrity
formula indicates its trustworthiness.
%
For example, the label \dcLabel{\https{amazon.com} $\land$
\https{chase.com}}{\https{amazon.com} $\lor$ \https{chase.com}}
specifies that the context may contain data that is sensitive to both
\https{amazon.com} and \https{chase.com}, and, that the compartment
may have been influenced by data from one (or both) of these origins.
%
Indeed, the integrator's compartment for the third-party mashup of
Section~\ref{sec:motivating-examples} will have such a label after
reading the Amazon and bank XHR responses.
%
In the rest of the paper, we focus on secrecy and simplify notation by
excluding the integrity formula; we include integrity explicitly when
it is non-obvious.
...
%% Labels dictate where information can flow
