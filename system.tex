...
\Red{EZY: suggest condensing this into a single paragraph, no need
to repeat what we're going to say; we've already laid out the outline
...
Section~\ref{sec:motivating-examples}. \Red{EZY: suggest cutting, we already said this: Throughout this paper, when
...
When one browsing context sends sensitive information to another, 
a sending context can use \emph{labeled communication} to confine the
potentially untrusted code receiving the information. This enables symmetric confinement, whose
...
%
Labeled contexts naturally extend today's browser contexts which are used to
...
otherwise trusted by the target: the source must be at least
as trustworthy as the target.
%
In such cases, we say that such a target label ``subsumes'' the source
label.
